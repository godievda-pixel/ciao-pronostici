...
export function multitournamentRuntimeSource() {
  ...
  function __cwMtKickoff(value){const time=Date.parse(value||'');if(!Number.isFinite(time))return 'Время уточняется';try{return new Intl.DateTimeFormat('ru-RU',{hour:'2-digit',minute:'2-digit',timeZone:'Europe/Rome'}).format(new Date(time))}catch(_e){return 'Время уточняется'}}
  ...
  function __cwMtGroups(){const matches=Array.isArray(__cwMtPayload?.matches)?__cwMtPayload.matches:[],map=new Map();for(const match of matches){const key=String(match?.stageKey||'matches');if(!map.has(key))map.set(key,{key,label:String(match?.stageLabel||'Матчи'),order:Number(match?.stageOrder??9999),matches:[]});map.get(key).matches.push(match)}return [...map.values()].sort((a,b)=>a.order-b.order||a.label.localeCompare(b.label,'ru'))}
  ...
  try{const styleId='cwmt-production-style';if(!document.getElementById(styleId)){const style=document.createElement('style');style.id=styleId;style.textContent='#ciao-miniapp-root .cwmt-hub{padding-top:2px}#ciao-miniapp-root .cwmt-hub-title{margin-bottom:12px}#ciao-miniapp-root .cwmt-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}#ciao-miniapp-root .cwmt-tournament-card{position:relative;min-height:94px;border:1px solid rgba(255,255,255,.12);border-radius:20px;padding:16px 15px;display:flex;align-items:flex-end;justify-content:space-between;text-align:left;color:#fff;font:inherit;font-weight:900;font-size:15px;line-height:1.12;overflow:hidden;box-shadow:0 12px 28px rgba(0,0,0,.18);transition:transform .12s ease,border-color .15s ease}#ciao-miniapp-root .cwmt-tournament-card:active{transform:scale(.985)}#ciao-miniapp-root .cwmt-tournament-card i{font-style:normal;font-size:21px;line-height:1;opacity:.82}#ciao-miniapp-root .cwmt-tournament-card--wide{grid-column:1/-1;min-height:104px}#ciao-miniapp-root [data-cwmt-theme="serie-a"]{--cwmt-a:#3150ff;--cwmt-b:#0b2f88;background:radial-gradient(circle at 88% 0%,rgba(92,164,255,.35),transparent 40%),linear-gradient(135deg,var(--cwmt-a),var(--cwmt-b))}#ciao-miniapp-root [data-cwmt-theme="coppa"]{--cwmt-a:#159457;--cwmt-b:#9f2435;background:linear-gradient(120deg,rgba(21,148,87,.52),transparent 44%),linear-gradient(240deg,rgba(159,36,53,.56),transparent 48%),#11151d}#ciao-miniapp-root [data-cwmt-theme="champions"]{--cwmt-a:#3d4ec9;--cwmt-b:#33246f;background:radial-gradient(circle at 85% 10%,rgba(112,126,255,.42),transparent 34%),linear-gradient(135deg,var(--cwmt-a),var(--cwmt-b))}#ciao-miniapp-root [data-cwmt-theme="europa"]{--cwmt-a:#e66a13;--cwmt-b:#3a1b08;background:radial-gradient(circle at 88% 8%,rgba(255,126,31,.42),transparent 35%),linear-gradient(135deg,var(--cwmt-a),var(--cwmt-b))}#ciao-miniapp-root [data-cwmt-theme="conference"]{--cwmt-a:#28a968;--cwmt-b:#0b3b28;background:radial-gradient(circle at 88% 8%,rgba(71,216,137,.35),transparent 35%),linear-gradient(135deg,var(--cwmt-a),var(--cwmt-b))}#ciao-miniapp-root .cwmt-cover{position:relative;min-height:92px;border-radius:21px;padding:17px 16px 15px 58px;margin-bottom:12px;display:flex;align-items:flex-end;overflow:hidden;border:1px solid rgba(255,255,255,.12);box-shadow:0 14px 30px rgba(0,0,0,.18)}#ciao-miniapp-root .cwmt-cover h2{margin:0;color:#fff;font-size:24px;line-height:1.03;letter-spacing:-.035em}#ciao-miniapp-root .cwmt-back{position:absolute;left:13px;top:13px;width:36px;height:36px;border-radius:12px;border:1px solid rgba(255,255,255,.18);background:rgba(4,10,30,.28);color:#fff;font-size:28px;line-height:1}#ciao-miniapp-root .cwmt-rounds .round-chip.active{background:linear-gradient(135deg,var(--cwmt-a,#3150ff),var(--cwmt-b,#0b2f88));border-color:rgba(255,255,255,.2)}#ciao-miniapp-root .cwmt-match-main{display:grid;grid-template-columns:minmax(0,1fr) 74px minmax(0,1fr);align-items:center;gap:8px}#ciao-miniapp-root .cwmt-team{appearance:none;border:0;background:transparent;color:inherit;padding:0;display:flex;align-items:center;gap:8px;min-width:0;text-align:left;font:inherit}#ciao-miniapp-root .cwmt-team--away{flex-direction:row-reverse;text-align:right}#ciao-miniapp-root .cwmt-team--link{cursor:pointer;border-radius:12px}#ciao-miniapp-root .cwmt-team--link:active{transform:scale(.985);background:rgba(49,80,255,.08)}#ciao-miniapp-root .cwmt-board-logo{width:34px;height:34px;object-fit:contain;flex:0 0 34px}#ciao-miniapp-root .cwmt-board-logo--empty{border-radius:50%;background:rgba(255,255,255,.07)}#ciao-miniapp-root .cwmt-match-center{text-align:center;min-width:0}#ciao-miniapp-root .cwmt-live{margin-top:3px;color:#ff526d;font-size:10px;font-weight:950;letter-spacing:.04em}#ciao-miniapp-root .cwmt-status{margin-top:3px;color:#91a2cc;font-size:10px;font-weight:800}#ciao-miniapp-root .cwmt-status--notice{margin:0;color:#f2b45f;font-size:11px}#ciao-miniapp-root .cwmt-kickoff{font-size:15px}#ciao-miniapp-root .cwmt-state{border:1px solid rgba(255,255,255,.09);border-radius:18px;background:rgba(255,255,255,.045);padding:18px;color:#91a2cc;text-align:center}#ciao-miniapp-root .cwmt-state b{display:block;color:#fff;margin-bottom:10px}#ciao-miniapp-root .cwmt-state button{border:0;border-radius:11px;background:#3150ff;color:#fff;padding:9px 13px;font:800 11px/1 inherit}@media(max-width:340px){#ciao-miniapp-root .cwmt-grid{grid-template-columns:1fr}#ciao-miniapp-root .cwmt-tournament-card--wide{grid-column:auto}}';document.head.appendChild(style)}}catch(_e){}
  /* /${MULTITOURNAMENT_PATCH_MARKER} */
`;
}
