...
async function resetProbe() {
  try {
    return await import('../scripts/probe-reset-contract.mjs');
  } catch (error) {
    assert.fail(`reset contract probe is missing: ${error?.code || error?.message || error}`);
  }
}
...
test('v23.3 reset probe source has no destructive network path', async () => {
  const source = await readFile(
    new URL('../scripts/probe-reset-contract.mjs', import.meta.url),
    'utf8',
  );
