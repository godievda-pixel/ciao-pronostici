...
test('Round 51 Home integration module imports cleanly after cutover', async () => {
  const moduleUrl = new URL('../src/v23.3/home-integration.mjs', import.meta.url);
  moduleUrl.searchParams.set('round51-import', String(Date.now()));
  const mod = await import(moduleUrl.href);
  assert.equal(typeof mod.createHomeRuntime, 'function');
  assert.equal(typeof mod.renderHomeTodaySection, 'function');
});
