...
function matchCard(match) {
  return `<article class="cw232-match-card" data-cw232-match="${esc(match?.matchId || '')}"><div class="cw232-match-card__teams"><div class="cw232-match-team cw232-match-team--home">${teamLogo(match?.homeTeam)}<strong>${esc(match?.homeTeam?.name || '—')}</strong></div><div class="cw232-match-card__center"><span class="cw232-match-card__score">${esc(scoreText(match) || '—')}</span><small>${esc(matchStatus(match))}</small></div><div class="cw232-match-team cw232-match-team--away">${teamLogo(match?.awayTeam)}<strong>${esc(match?.awayTeam?.name || '—')}</strong></div></div></article>`;
}
...
.cw232-group-tabs{display:flex;gap:7px;max-width:100%;overflow-x:auto;scrollbar-width:none;padding:0 0 13px}.cw232-group-tabs::-webkit-scrollbar{display:none}.cw232-group-tabs button{flex:0 0 auto;min-width:52px;min-height:38px;padding:0 13px;border:1px solid rgba(255,255,255,.1);border-radius:13px;background:rgba(255,255,255,.05);color:rgba(226,233,255,.72);font:800 11px/1 inherit}.cw232-group-tabs button[aria-selected='true']{background:linear-gradient(180deg,#3150ff,#142bd6);border-color:rgba(109,134,255,.58);color:#fff;box-shadow:0 6px 18px rgba(23,48,207,.24)}.cw232-group-panel[hidden]{display:none!important}
