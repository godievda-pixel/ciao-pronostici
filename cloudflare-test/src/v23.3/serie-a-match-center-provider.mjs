...
function normalizeLineupSideAliases(value) {
  ...
  const combined = firstArray(side.players);
  const explicitStarters = firstArray(side.starters);
  let starters = explicitStarters || combined || [];
  let substitutes = firstArray(
    side.substitutes,
    side.bench,
    side.subs,
    side.reserves,
  ...
  if (!substitutes && combined?.some(player => player?.starter === false || player?.is_starter === false)) {
    substitutes = combined.filter(player => player?.starter === false || player?.is_starter === false);
    if (!explicitStarters) starters = combined.filter(player => player?.starter !== false && player?.is_starter !== false);
  }
  return {
    ...side,
    starters,
