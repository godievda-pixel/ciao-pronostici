...
function statsStyles() {
  return `<style data-cw233-mc-stats-parity-style>
    ...
    @media(max-width:339px){.cw233-mc-key-metrics{gap:4px}.cw233-mc-key-metric{padding-left:4px;padding-right:4px}.cw233-mc-pitch{width:min(100%,232px)}.cw233-mc-shot-row{grid-template-columns:32px minmax(0,1fr) 36px;gap:6px}}
  </style>`;
}
...
export function renderMatchCenterStats(section = {}, context = {}) {
  ...
  return `${statsStyles()}<section class="cw233-mc-stats" data-cw233-mc-stats>
    ...
    ${keyMetrics(home, away)}${primary}${extended}${shotMap(shots, homeName, awayName)}${shotList(shots, homeName, awayName)}
  </section>`;
}

export { STAT_DEFINITIONS, PRIMARY_STAT_DEFINITIONS, EXTENDED_STAT_DEFINITIONS, shotClock, shotMarker, outcomeLabel };
