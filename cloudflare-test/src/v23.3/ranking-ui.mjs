...
function favoriteClub(row={},hero=false){
  ...
  const name=text(team?.name)||'Любимый клуб';
  const classes=`cw233-ranking-avatar${hero?' cw233-ranking-avatar--hero':''}`;
  if(crest)return `<div class="${classes}" title="${esc(name)}"><img class="cw233-ranking-club-logo" src="${esc(crest)}" alt="${esc(name)}" loading="eager" decoding="async"></div>`;
  return `<div class="${classes} cw233-ranking-club-placeholder" title="${esc(name)}" aria-label="${esc(name)}">⚽</div>`;
}
...
function heroHtml(){const tgUser=telegramUser();const name=resolveRankingDisplayName(me,tgUser);const subtitle=usernameLine(me,tgUser);const rank=Number(me?.position);const points=Number(me?.points)||0;return `<div class="hero cw233-ranking-hero"><div class="cw233-ranking-identity">${favoriteClub(me,true)}<div class="cw233-ranking-identity-copy"><span class="cw233-ranking-kicker">УЧАСТНИК</span><h2>${esc(name)}</h2><p>${esc(subtitle)}</p></div></div><div class="cw233-ranking-hero-stats"><div class="cw233-ranking-stat"><strong>${rank>0?`#${rank}`:'—'}</strong><span>место</span></div><div class="cw233-ranking-stat"><strong>${points}</strong><span>${esc(rankingPointsUnit(points))}</span></div></div></div>`;}
