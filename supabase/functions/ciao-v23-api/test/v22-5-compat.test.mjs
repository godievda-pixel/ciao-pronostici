...
test('specialized stable v22.5 endpoints resolve deterministically', () => {
  ...
  assert.deepEqual(legacyRouteFor('ciao-match-center', { match_id:123 }), { kind:'match_center', action:'load' });
  assert.deepEqual(legacyRouteFor('ciao-match-center-fast-v3', { match_id:123 }), { kind:'match_center', action:'load' });
  assert.deepEqual(legacyRouteFor('ciao-match-summary-fast-v2', { match_id:123 }), { kind:'match_summary', action:'load' });
  ...
  assert.deepEqual(legacyRouteFor('ciao-miniapp-api', {}), { kind:'diagnostic', action:'load' });
});
