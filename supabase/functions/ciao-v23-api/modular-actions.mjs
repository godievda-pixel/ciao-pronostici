...
function predictionCompetition(body = {}) {
  ...
  const prefixed = items.map(item => text(item?.match_id ?? item?.matchId)).map(id => id.includes(':') ? id.split(':', 1)[0].toLowerCase() : '').filter(Boolean);
...
function validateExternalPredictionPayload(body, key) {
  ...
  for (const item of items) normalizeCanonicalMatchId(key, item?.match_id ?? item?.matchId);
}

export function isModularAction(action) { return MODULAR_ACTION_SET.has(text(action)); }
...
export function createModularActionRouter(deps = {}) {
  return async function routeModularAction(actionValue, body = {}, context = {}) {
    ...
    if (action === 'modular_matches') return await requireHandler(deps, 'loadMatches')({ ...body, competition:competition(body.competition) }, context);
    if (action === 'modular_standings') return await requireHandler(deps, 'loadStandings')({ ...body, competition:competition(body.competition) }, context);
    if (action === 'modular_favorite') return await requireHandler(deps, 'loadFavorite')(body, context);
    if (action === 'modular_predictions') {
      ...
      return await requireHandler(deps, 'loadPredictions')({ ...body, mode, competition:competition(body.competition, { optional:true }) }, context);
    }
    ...
    return await requireHandler(deps, 'loadMatchCenter')({ ...body, competition:key, match_id:matchId, section }, context);
  };
}
